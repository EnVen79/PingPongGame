...
#define BUTTON_1 2              // Пин кнопки первого игрока
#define BUTTON_2 3               // Пин кнопки второго игрока

int ballX = random(7, 9);
int ballY = random(7, 9);                          // Мяч появляется в верхней или нижней зоне
int ballDirX = 1-random(0, 2)*2;  // Направление по горизонтали
int ballDirY = 1-random(0, 2)*2;  // Направление по вертикали
...
void resetBall()  // Функция возвращения мяча
{
  ballX = random(7, 9);
  ballY = random(7, 9);
  ballDirX = 1-random(0, 2)*2;  // Направление по горизонтали
  ballDirY = 1-random(0, 2)*2;
}
...
void loop() {
  ...
  if ((ballY == 1 || ballY == HEIGHT) && (ballX >= paddleX1 && ballX <= paddleX1 + PADDLE_SIZE)) 
  {
    ballDirY *= -1;
  } 
  else if ((ballY == HEIGHT - 2 || ballY == HEIGHT) && (ballX >= paddleX2 && ballX <= paddleX2 + PADDLE_SIZE)) 
  {
    ballDirY *= -1;
  }
